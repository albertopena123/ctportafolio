...
$( document ).ready(function() {
    ...
    tabla = $("#t_facultades").DataTable({
        "processing": true,
        "serverSide": true,
        "pageLength": 50,
        "order": [],
        "ajax": {
            "url":  default_server+"/json/facultades",
            "type": "POST",
            "data": function ( d ) { },
        ...
        "columns": [    
            ...
            { "data": "prt_dept_academicos_count", "searchable": false, className: "w-1",
                render: function ( data, type, full ) {                      
                    return data;
                }        
            },         
...
function guardar() 
{
    if(validar('#form_editar'))
    {
        ...
        if(editItem == 0)//nuevo
        {
            $.ajax({
                type: "POST",
                url: default_server+"/json/facultades/nuevo",
                data: {
                    codigo: $("#codigo").val(),
        ...
        else
        {
            $.ajax({
                type: "POST",
                url: default_server+"/json/facultades/"+editItem+"/modificar",
                data: {    
                    codigo: $("#codigo").val(),
                    abreviatura: $("#abreviatura").val(),               
                    nombre: $("#nombre").val(),
...
function eliminar(iditem) {
    if(confirm('Esta seguro que desea eliminar?'))
    {
        ...
        $.ajax({
            type: "POST",
            url: default_server+"/json/facultades/"+iditem+"/eliminar",        
            success: function(result){  
                $("#cargando_pagina").hide(); 
                alerta(result.message, true); 
                tabla.ajax.reload();
