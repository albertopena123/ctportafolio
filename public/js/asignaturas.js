...
$( document ).ready(function() {
    ...
    tabla = $("#t_asignaturas").DataTable({
        "processing": true,
        "serverSide": true,
        "pageLength": 50,
        "order": [],
        "ajax": {
            "url":  default_server+"/json/asignaturas",
            "type": "POST",
            "data": function ( d ) {
                d.prt_facultad_id = $("#facultad_select").val(); 
                d.prt_dept_academico_id = $("#departamento_select").val(); 
            },
        ...
        "columns": [       
            ...
            { "data": "prt_academ_cargas_count", "searchable": false, className: "w-1",
                render: function ( data, type, full ) {       
                    return data;
                }        
            },                    
    ...
    $('#facultad_select').on('change', function() {
        var prt_facultad_id = $('#facultad_select').val();
        $('#departamento_select').html('<option value="0">TODOS</option>');
        if(prt_facultad_id != 0) {
            for (let i = 0; i < departamentos.length; i++) {
                if(departamentos[i].prt_facultad_id == prt_facultad_id) {
                    $('#departamento_select').append('<option value="'+departamentos[i].id+'">'+departamentos[i].nombre+'</option>');
                }
            }
        }  
        $("#departamento_select").trigger('change');      
    });
    ...
    $('#prt_facultad_id').on('change', function() {        
        var prt_facultad_id = $('#prt_facultad_id').val();
...
function guardar() 
{
    if(validar('#form_editar'))
    {
        ...
        if(editItem == 0)//nuevo
        {
            $.ajax({
                type: "POST",
                url: default_server+"/json/asignaturas/nuevo",
                data: {
        ...
        else
        {
            $.ajax({
                type: "POST",
                url: default_server+"/json/asignaturas/"+editItem+"/modificar",
                data: {                   
                    prt_facultad_id: $("#prt_facultad_id").val(),
                    prt_dept_academico_id: $("#prt_dept_academico_id").val(),
                    codigo: $("#codigo").val(),                    
...
function eliminar(iditem) {
    if(confirm('Esta seguro que desea eliminar?'))
    {
        ...
        $.ajax({
            type: "POST",
            url: default_server+"/json/asignaturas/"+iditem+"/eliminar",        
            success: function(result){  
                $("#cargando_pagina").hide(); 
                alerta(result.message, true); 
                tabla.ajax.reload();
