...
$( document ).ready(function() {
    ...
    tabla = $("#t_escuelas").DataTable({
        "processing": true,
        "serverSide": true,
        "pageLength": 50,
        "order": [],
        "ajax": {
            "url":  default_server+"/json/escuelas",
            "type": "POST",
            "data": function ( d ) {
...
function guardar() 
{
    if(validar('#form_editar'))
    {
        ...
        if(editItem == 0)//nuevo
        {
            $.ajax({
                type: "POST",
                url: default_server+"/json/escuelas/nuevo",
                data: {
        ...
        else
        {
            $.ajax({
                type: "POST",
                url: default_server+"/json/escuelas/"+editItem+"/modificar",
                data: {                   
                    prt_facultad_id: $("#prt_facultad_id").val(),
                    prt_dept_academico_id: $("#prt_dept_academico_id").val(),
                    codigo: $("#codigo").val(),
...
function eliminar(iditem) {
    if(confirm('Esta seguro que desea eliminar?'))
    {
        ...
        $.ajax({
            type: "POST",
            url: default_server+"/json/escuelas/"+iditem+"/eliminar",        
            success: function(result){  
                $("#cargando_pagina").hide(); 
                alerta(result.message, true); 
                tabla.ajax.reload();
